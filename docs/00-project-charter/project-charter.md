...
```mermaid
flowchart LR
    User["Пользователь"] --> UC1["Вести коллекцию фильмов"]
    User --> UC2["Искать фильм"]
    User --> UC3["Фиксировать просмотр"]
    Admin["Администратор"] --> UC4["Управлять жанрами"]
    UC1 --> UC5["Сохранять изменения"]
```
